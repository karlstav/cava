...
\n";
  ...
  while ((c = getopt(argc, argv, ":p:vh")) != -1) {
    ...
    case 'p': // argument: config path
      snprintf(configPath, PATH_MAX, "%s", optarg);
      break;
...
static void setup_tty_environment(struct config_params *cfg, int *inAtty,
                                  int *inAterminal) {
#ifndef _WIN32
...
static void start_audio_thread(struct config_params *cfg,
                               struct audio_data *audio, pthread_t *p_thread) {
  ...
  case INPUT_COREAUDIO:
    ...
    if (!strcmp(audio->source, "list")) {
      ...
      thr_id =
          pthread_create(p_thread, NULL, input_coreaudio_tap, (void *)audio);
#endif
    } else {
...
static void init_cava_buffers(struct cava_buffers *buf, int number_of_bars,
                              int output_channels, int audio_channels,
                              bool split_stereo) {
  buf->bars_left =
  ...
  buf->cava_out = (double *)malloc(number_of_bars * audio_channels /
  ...
  memset(buf->cava_out, 0,
         sizeof(double) * number_of_bars * audio_channels / output_channels);
...
static void free_cava_buffers(struct cava_buffers *buf, int audio_channels,
                              bool split_stereo) {
  if (audio_channels == 2) {
    free(buf->bars_left);
    free(buf->bars_right);
...
static void handle_keyboard_input(char *ch, struct config_params *cfg,
                                  char *configPath,
...
static void draw_x_axis(struct config_params *cfg, struct cava_plan *plan,
                        int lines, int remainder, int number_of_bars) {
    ...
    for (int n = 0; n < number_of_bars; n++) {
      ...
      if (output_mode == OUTPUT_NCURSES) {
        ...
        if (cut_off_frequency < 1000)
          mvprintw(lines, n * (cfg->bar_width + cfg->bar_spacing) + remainder,
                   "%-4d", freq_floor);
...
static int render_output(struct config_params *cfg, struct cava_buffers *buf,
  ...
  case OUTPUT_SDL:
    rc = draw_sdl(number_of_bars, cfg->bar_width, cfg->bar_spacing, remainder,
                  dimension_value, buf->bars, buf->previous_frame,
                  frame_time_msec, cfg->orientation, cfg->gradient);

    break;
#endif
#ifdef SDL_GLSL
  case OUTPUT_SDL_GLSL:
    rc = draw_sdl_glsl(number_of_bars, buf->bars_raw, buf->previous_bars_raw,
                       frame_time_msec, re_paint, cfg->continuous_rendering);
    break;
#endif
    ...
    if (cfg->orientation == ORIENT_SPLIT_H ||
      ...
      if (cfg->split_stereo) {
        ...
        for (int i = 0; i < number_of_bars / 2; i++) {
          ...
          buf->right_previous_frame[i] =
              buf->previous_frame[i + number_of_bars / 2];
        }
        if (cfg->orientation == ORIENT_SPLIT_H) {
          rc = draw_terminal_noncurses(buf->bars, buf->previous_frame,
                                       ORIENT_BOTTOM, cfg);
          rc = draw_terminal_noncurses(
              buf->right_bars, buf->right_previous_frame, ORIENT_TOP, cfg);
        }
        if (cfg->orientation == ORIENT_SPLIT_V) {
          rc = draw_terminal_noncurses(
              buf->right_bars, buf->right_previous_frame, ORIENT_LEFT, cfg);
          rc = draw_terminal_noncurses(buf->bars, buf->previous_frame,
                                       ORIENT_RIGHT, cfg);
        }
  ...
  case OUTPUT_NCURSES:
    ...
    rc = draw_terminal_ncurses(inAtty, dimension_value / 8, *dimension_bar,
                               ...
                               remainder, buf->bars, buf->previous_frame,
                               cfg->gradient, cfg->xaxis, cfg->orientation);
    break;
#endif
  case OUTPUT_RAW:
#ifndef _WIN32
    rc = print_raw_out(number_of_bars, fp, cfg->raw_format, cfg->bit_format,
                       cfg->ascii_range, cfg->bar_delim, cfg->frame_delim,
                       buf->bars);
#else
...
#ifndef _WIN32
    rc = print_ntk_out(number_of_bars, fp, cfg->bit_format, cfg->bar_width,
...
static void process_audio_chunk(struct audio_data *audio,
                                struct config_params *cfg,
                                struct cava_buffers *buf,
                                struct cava_plan *plan, int high_framerate,
                                int samples_per_frame, int audio_channels,
                                int number_of_bars) {
...
static int format_and_filter_bars(struct cava_buffers *buf,
                                  struct config_params *cfg,
                                  struct terminal_dimensions *termDim,
  ...
  if (!cfg->waveform) {
    ...
    if (cfg->monstercat) {
      if (audio_channels == 2) {
        ...
        buf->bars_raw =
            monstercat_filter(buf->bars_raw, number_of_bars, cfg->waves,
...
static void sync_framerate(struct framerate_sync *fs, long long frame_time_ns) {
  if (output_mode != OUTPUT_SDL && output_mode != OUTPUT_SDL_GLSL) {
#ifdef _WIN32
    int frame_time_msec = (int)(frame_time_ns / 1000000);
    LARGE_INTEGER t2;
    QueryPerformanceCounter(&t2);
    double elapsedTime =
        (t2.QuadPart - fs->t1.QuadPart) * 1000.0 / fs->frequency.QuadPart;
    ...
    double elapsedTimens = (t2.tv_sec - fs->t1.tv_sec) * 1000000000.0 +
                           (t2.tv_nsec - fs->t1.tv_nsec);

    int sleep_time_ns =
        frame_time_ns - (int)elapsedTimens - (int)fs->wakeupTimens;
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    if (output_mode == OUTPUT_SDL) {
      init_sdl_window(cfg.sdl_width, cfg.sdl_height, cfg.sdl_x, cfg.sdl_y,
                      cfg.sdl_full_screen);
...
#ifdef SDL_GLSL
    if (output_mode == OUTPUT_SDL_GLSL) {
      init_sdl_glsl_window(cfg.sdl_width, cfg.sdl_height, cfg.sdl_x, cfg.sdl_y,
                           cfg.sdl_full_screen, cfg.vertex_shader,
                           cfg.fragment_shader);
      termDim.height = cfg.sdl_height;
      termDim.width = cfg.sdl_width;
      *termDim.dim_val = 1;
    ...
    while (!reloadConf) { // jumping back to this loop means that you resized
      ...
      case OUTPUT_NCURSES:
        init_terminal_ncurses(cfg.color, cfg.bcolor, cfg.col, cfg.bgcol,
                              cfg.gradient, cfg.gradient_count,
                              cfg.gradient_colors, &termDim.width, &lines);
        if (cfg.xaxis != NONE)
          lines--;
        termDim.height = lines;
        *termDim.dim_val *= 8; // we have 8 times as much height due to using
      ...
      case OUTPUT_SDL:
        init_sdl_surface(&termDim.width, &termDim.height, cfg.color, cfg.bcolor,
                         cfg.gradient, cfg.gradient_count, cfg.gradient_colors);
        break;
#endif
#ifdef SDL_GLSL
      // output: get sdl window size
      case OUTPUT_SDL_GLSL:
        init_sdl_glsl_surface(&termDim.width, &termDim.height, cfg.color,
                              cfg.bcolor, cfg.bar_width, cfg.bar_spacing,
                              cfg.gradient, cfg.gradient_count,
                              cfg.gradient_colors);
        break;
#endif
      ...
      if (cfg.fixedbars) {
        ...
        if (number_of_bars * cfg.bar_width + cfg.fixedbars * cfg.bar_spacing -
                cfg.bar_spacing >
            termDim.width) {
          cleanup();
          fprintf(
              ...
              (termDim.width - cfg.bar_spacing) /
                  (cfg.bar_width + cfg.bar_spacing));
          audio.terminate = 1;
          exit(EXIT_FAILURE);
        }
        ...
        number_of_bars = (*termDim.dim_bar + cfg.bar_spacing) /
                         (cfg.bar_width + cfg.bar_spacing);
      ...
      if (output_mode == OUTPUT_NONCURSES) {
        init_terminal_noncurses(
            inAtty, cfg.color, cfg.bcolor, cfg.col, cfg.bgcol, cfg.gradient,
            ...
            number_of_bars, termDim.width, lines, cfg.bar_width,
            cfg.orientation, cfg.blendDirection);
      }
      if ((cfg.orientation == ORIENT_SPLIT_H ||
           cfg.orientation == ORIENT_SPLIT_V) &&
          cfg.split_stereo) {
      ...
      if (cfg.userEQ_enabled && (number_of_bars / output_channels > 0)) {
        userEQ_keys_to_bars_ratio =
            (double)(((double)cfg.userEQ_keys) /
                     ((double)(number_of_bars / output_channels)));
      }

      struct cava_plan *plan =
          cava_init(number_of_bars / output_channels, audio.rate,
                    audio.channels, cfg.autosens, cfg.noise_reduction,
                    cfg.lower_cut_off, cfg.upper_cut_off);
      ...
      init_cava_buffers(&buf, number_of_bars, output_channels, audio_channels,
                        cfg.split_stereo);
      ...
      long long frame_time_ns =
          (long long)(1000000000.0 / (double)cfg.framerate);
      if (frame_time_ns < 1)
        frame_time_ns = 1;
      ...
      while (!resizeTerminal) {
        ...
        handle_keyboard_input(&ch, &cfg, configPath, &termDim, &resizeTerminal,
                              &reloadConf);
        ...
        process_audio_chunk(&audio, &cfg, &buf, plan, high_framerate,
                            samples_per_frame, audio_channels, number_of_bars);

        int re_paint = format_and_filter_bars(
            &buf, &cfg, &termDim, raw_number_of_bars, number_of_bars,
            audio_channels, output_channels, userEQ_keys_to_bars_ratio);
...
#ifndef _WIN32
        int rc = render_output(&cfg, &buf, number_of_bars, remainder,
                               *termDim.dim_val, termDim.dim_bar,
                               frame_time_msec, re_paint, inAtty, fp);
#else
        int rc = render_output(&cfg, &buf, number_of_bars, remainder,
                               *termDim.dim_val, termDim.dim_bar,
                               frame_time_msec, re_paint, inAtty, hFile);
#endif
        ...
        if (cfg.output == OUTPUT_SDL_GLSL) {
          memcpy(buf.previous_bars_raw, buf.bars_raw,
                 number_of_bars * sizeof(float));
        }
        ...
        sync_framerate(&fs, frame_time_ns);
