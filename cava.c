...
static bool get_file_state(const char *path, uint64_t *mtime_ns,
                           long long *size) {
#ifdef _WIN32
  struct _stat st;
  if (_stat(path, &st) != 0)
...
\n";
  ...
  while (1) {
    ...
    if (p.live_config) {
      has_config_state =
          get_file_state(configPath, &config_mtime_ns, &config_size);
    }
    ...
    while (!reloadConf) { // jumping back to this loop means that you resized
      ...
      long long frame_time_ns =
          (long long)(1000000000.0 / (double)effective_framerate);
      if (frame_time_ns < 1)
        frame_time_ns = 1;
        ...
        if (should_reload) {
          ...
          if (!should_quit && output_mode == OUTPUT_SDL_GLSL) {
            ...
            if (load_config(configPath, &p_new, &error)) {
              ...
              can_soft_reload &=
                  (p_new.horizontal_stereo == p.horizontal_stereo);
              ...
              if (can_soft_reload) {
                ...
                init_sdl_glsl_surface(
                    &width, &height, p_new.color, p_new.bcolor, p_new.bar_width,
...
#ifdef SDL_GLSL
        if (output_mode == OUTPUT_SDL_GLSL &&
            startup_frames < startup_limit_frames) {
          float denom =
              (startup_limit_frames > 0) ? (float)startup_limit_frames : 1.0f;
        ...
        case OUTPUT_NONCURSES:
          if (p.orientation == ORIENT_SPLIT_H) {
            ...
            if (p.horizontal_stereo) {
              ...
              rc = draw_terminal_noncurses(
                  inAtty, lines, width, number_of_bars, p.bar_width,
                  p.bar_spacing, remainder, bars, previous_frame, p.gradient,
                  p.horizontal_gradient, x_axis_info, ORIENT_BOTTOM, 1);
              rc = draw_terminal_noncurses(
                  inAtty, lines, width, number_of_bars, p.bar_width,
                  p.bar_spacing, remainder, bars, previous_frame, p.gradient,
                  p.horizontal_gradient, x_axis_info, ORIENT_TOP, 1);
            }
          } else {
            rc = draw_terminal_noncurses(
                inAtty, lines, width, number_of_bars, p.bar_width,
                p.bar_spacing, remainder, bars, previous_frame, p.gradient,
                p.horizontal_gradient, x_axis_info, p.orientation, 0);
          }
          break;
        case OUTPUT_NCURSES:
#ifdef NCURSES
