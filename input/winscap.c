...
void get_audio_device_by_name(IMMDeviceEnumerator *pEnumerator,
  ...
  HRESULT hr = pEnumerator->lpVtbl->EnumAudioEndpoints(
      pEnumerator, eRender, DEVICE_STATE_ACTIVE, &pDeviceCollection);
  if (FAILED(hr)) {
...
void *input_winscap(void *data) {
  ...
  while (!audio->terminate) {
    ...
    hr = pDevice->lpVtbl->Activate(pDevice, &IID_IAudioClient, CLSCTX_ALL, 0,
                                   (void **)&pClient);
    ...
    if (FAILED(hrInit)) {
      ...
      if (SUCCEEDED(hr) && pProps) {
        hr = pProps->lpVtbl->GetValue(pProps, &PKEY_Device_FriendlyName,
                                      &varName);
        if (SUCCEEDED(hr) && varName.pwszVal) {
          fwprintf(stderr, L"Failed to open: %s\n", varName.pwszVal);
        } else {
          fwprintf(stderr,
                   L"Failed to open device (GetValue failed: 0x%08lx)\n", hr);
        }
      } else {
        fwprintf(stderr,
                 L"Failed to open device (OpenPropertyStore failed: 0x%08lx)\n",
                 hr);
      }
      PropVariantClear(&varName);
      if (pProps)
    ...
    while (!audio->terminate) {
      ...
      while (packetLength) {
        ...
        hr = pCapture->lpVtbl->GetBuffer(pCapture, &pData, &numFramesAvailable,
                                         &flags, 0, 0);
        if (FAILED(hr)) {
