...
static bool set_format(int fd, struct audio_data *audio) {
  ...
  if (fmts_8bits & fmt)
    ...
    fprintf(stderr,
            __FILE__
            ": No support for 8, 16, 24 or 32 bits in OSS source '%s'.\n",
            audio->source);
    return false;
  }
...
void *input_oss(void *data) {
  ...
  if (!set_format(fd, audio) || !set_channels(fd, audio) ||
      !set_rate(fd, audio))
    goto cleanup;
