...
static bool set_channels(struct jack_data *jack) {
  ...
  static const char *port_name[MAX_CHANNELS][MAX_CHANNELS] = {
      /* mono */ {"M", NULL},
      /* stereo */ {"L", "R"}};
  ...
  for (ch = 0; ch < channels; ++ch) {
    if ((jack->port[ch] = jack_port_register(
             jack->client, port_name[chtype][ch], port_type, flags, 0)) == NULL)
      break;
  }
  ...
  if (ch < channels) {
    ...
    for (ch = 0; ch < channels; ++ch) {
      ...
      if ((err = jack_port_rename(jack->client, jack->port[ch],
                                  port_name[chtype_new][ch])) != 0) {
        fprintf(stderr,
                __FILE__ ": jack_port_rename('%s', '%s') failed: 0x%x\n",
...
static int on_buffer_size(jack_nframes_t nframes, void *arg) {
  ...
  if (jack->nframes != nframes) {
    fprintf(stderr, __FILE__
            ": Unexpected change of JACK port buffersize! Aborting!\n");
    jack->shutdown = 1;
    return 1;
  }
...
static int on_sample_rate(jack_nframes_t nframes, void *arg) {
  ...
  if (jack->audio->rate != nframes) {
    fprintf(stderr,
            __FILE__ ": Unexpected change of JACK sample rate! Aborting!\n");
    jack->shutdown = 1;
    return 1;
  }
...
static bool auto_connect(struct jack_data *jack) {
  ...
  if ((ports = jack_get_ports(jack->client, NULL, type_name_pattern, flags)) ==
    ...
    fprintf(
        stderr, __FILE__
        ": jack_get_ports() failed: No physical terminal input-ports found!\n");
    goto cleanup;
  }
...
void *input_jack(void *data) {
  ...
  static const struct timespec rqtp = {.tv_sec = 0,
                                       .tv_nsec = 1000000}; // 1ms nsleep
  ...
  if ((audio->channels > 1) &&
      ((jack.buf = malloc(jack.nframes * audio->channels * sizeof(sample_t))) ==
       NULL)) {
  ...
  if (audio->autoconnect > 0) {
    if (audio->autoconnect == 1)
      ...
      if ((err = jack_set_graph_order_callback(jack.client, on_graph_order,
                                               &jack)) != 0) {
        fprintf(stderr,
                __FILE__ ": jack_set_graph_order_callback() failed: 0x%x\n",
                err);
        goto cleanup;
      }
    }
