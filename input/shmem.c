...
void *input_shmem(void *data) {
  ...
  if (fd < 0) {
    ...
    mmap_area =
        mmap(NULL, sizeof(vis_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  ...
  while (!audio->terminate) {
    ...
    if (mmap_area->running) {
      ...
      // whole buffer.
      for (int i = 0; i < buf_frames / (fftw_frames * 2);
           i += fftw_frames * 2) {
        for (int n = 0; n < fftw_frames * 2; n++) {
          buf[n] = mmap_area->buffer[n + i];
        }
        write_to_cava_input_buffers(fftw_frames * 2, (unsigned char *)buf,
                                    audio);
      }
      nanosleep(&req, NULL);
    } else {
      write_to_cava_input_buffers(fftw_frames * 2,
                                  (unsigned char *)silence_buffer, audio);
      nanosleep(&req, NULL);
    }
  }
  ...
  if (fd > 0) {
    if (close(fd) != 0) {
      fprintf(stderr, "Could not close file descriptor %d: %s", fd,
              strerror(errno));
    }
  } else {
