...
# Manual sensitivity in %. If autosens is enabled this will not be used.
# 200 means double height. Accepts only non-negative values.
; sensitivity = 100

# Scaling can be 'linear' (default) or 'decibel'. Linear scaling provides a clear visualisation of music when using
# fewer bars (less than 100). While decibel scaling more correctly resembles how sound is perceived.
; scaling = linear
...
# Audio capturing method. Possible methods are: 'fifo', 'portaudio', 'coreaudio', 'pipewire', 'alsa', 'pulse', 'sndio', 'oss', 'jack' or 'shmem'
# Defaults to 'oss', 'pipewire', 'sndio', 'jack', 'pulse', 'coreaudio', 'alsa', 'portaudio' or 'fifo', in that order, dependent on what support cava was built with.
# On Mac it defaults to 'coreaudio', 'portaudio' or 'fifo'
...
#
# For coreaudio 'source' can be:
#   'auto' (default output device), 'auto_input' (default input device), 'list',
#   a 1-based device index from 'list', or an exact device name.
#
# On macOS 14.2+ with tap support compiled in, coreaudio can capture system output
# without a loopback interface using:
#   'tap' (stereo system mix) or 'tap_mono' (mono system mix)
#
...
# Orientation of the visualization. Can be 'bottom', 'top', 'left', 'right',
# 'horizontal' (bars go up and down from center) or 'vertical' (bars go left and right).
# Default is 'bottom'.
...
# Split channels when using 'horizontal' or 'vertical' orientation.
# Set 'split_stereo' to 1 to have left channel bars at top (left) and right channel at bottom (right).
# Set 'left_bottom' to 0 to have right channel at top and left channel at bottom for 'horizontal' orientation.
; split_stereo = 0
; left_bottom = 1
...
# Not supported with decibel scaling.
; 1 = 1 # bass
; 2 = 1
; 3 = 1 # midtone
; 4 = 1
