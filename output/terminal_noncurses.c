...
int init_terminal_noncurses(
    ...
    int bgcol, int gradient, int gradient_count, char **gradient_color_strings,
  ...
  if (horizontal_gradient && gradient) {
    twodim_gradient_colors = (struct colors *)malloc((number_of_bars * lines) *
...
int draw_terminal_noncurses(int tty, int lines, int width, int number_of_bars,
                            int bar_width, int bar_spacing, int rest,
                            int bars[], int previous_frame[], int gradient,
                            int horizontal_gradient, int x_axis_info,
  ...
  for (int current_line = lines - 1; current_line >= 0; current_line--) {
    ...
    for (int i = 0; i < number_of_bars; i++) {
      ...
      if ((current_cell < 1 && prev_cell < 1) ||
          (current_cell > 7 && prev_cell > 7) || (current_cell == prev_cell)) {
        same_bar++;
      } else {
        if (tty) {
          ...
          if (same_bar > 0) {
            cx +=
                snprintf(ttyframe_buffer + cx, ttybuf_length - cx, "\033[%dC",
          ...
          if (current_cell > 0) {
            ...
            if (gradient && horizontal_gradient) {
              if (orientation == ORIENT_BOTTOM) {
                cx += snprintf(
                    ttyframe_buffer + cx, ttybuf_length - cx,
                    "\033[38;2;%d;%d;%dm",
          ...
          if (same_bar > 0) {
            cx +=
                swprintf(frame_buffer + cx, buf_length - cx, L"\033[%dC",
                         (bar_width + bar_spacing) * same_bar); // move forward
            same_bar = 0;
          }
