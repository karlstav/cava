...
// general: keyboard controls
                ...
                */
                ...
                audio_raw_fetch(&audio_raw, &p, plan, &re_paint);
...
// output: draw processed input
...
#ifdef SDL
                case OUTPUT_SDL:
                    rc = draw_sdl(audio_raw.number_of_bars, p.bar_width, p.bar_spacing, audio_raw.remainder,
                                  *audio_raw.dimension_value, audio_raw.bars, audio_raw.previous_frame, frame_time_msec,
                ...
                case OUTPUT_SDL_GLSL:
                    rc = draw_sdl_glsl(audio_raw.number_of_bars, audio_raw.bars_raw, frame_time_msec, re_paint,
                                       p.continuous_rendering);
                    break;
#endif
                case OUTPUT_NONCURSES:
                    rc = draw_terminal_noncurses(p.inAtty, audio_raw.lines, audio_raw.width, audio_raw.number_of_bars, p.bar_width,
                                                 p.bar_spacing, audio_raw.remainder, audio_raw.bars, audio_raw.previous_frame,
                                                 p.gradient, p.x_axis_info);
                    break;
                case OUTPUT_NCURSES:
#ifdef NCURSES
                    rc = draw_terminal_ncurses(p.inAtty, *audio_raw.dimension_value / 8, *audio_raw.dimension_bar,
                                               audio_raw.number_of_bars, p.bar_width, p.bar_spacing,
                                               audio_raw.remainder, audio_raw.bars, audio_raw.previous_frame, p.gradient,
                ...
                case OUTPUT_NORITAKE:
                    rc = print_ntk_out(audio_raw.number_of_bars, p.fp, p.bit_format, p.bar_width, p.bar_spacing,
                                       p.bar_height, audio_raw.bars);
                    break;
                ...
                memcpy(audio_raw.previous_frame, audio_raw.bars, audio_raw.number_of_bars * sizeof(int));
