...
int audio_raw_init(struct audio_data* audio,
  ...
  switch (prm->output) {
    ...
    case OUTPUT_NCURSES:
      init_terminal_ncurses(prm->color, prm->bcolor, prm->col, prm->bgcol, prm->gradient,
                            prm->gradient_count, prm->gradient_colors, &audio_raw->width, &audio_raw->lines);
      if (prm->xaxis != NONE)
        audio_raw->lines--;
      audio_raw->height = audio_raw->lines;
      *audio_raw->dimension_value *= 8; // we have 8 times as much height due to using 1/8 block characters
      break;
#endif
#ifdef SDL
    // output: get sdl window size
    case OUTPUT_SDL:
      init_sdl_surface(&audio_raw->width, &audio_raw->height, prm->color, prm->bcolor, prm->gradient, prm->gradient_count,
                       prm->gradient_colors);
      break;
#endif
#ifdef SDL_GLSL
    ...
    case OUTPUT_SDL_GLSL:
      init_sdl_glsl_surface(&audio_raw->width, &audio_raw->height, prm->color, prm->bcolor, prm->bar_width,
                            prm->bar_spacing, prm->gradient, prm->gradient_count,
                            prm->gradient_colors);
      break;
#endif
    case OUTPUT_NONCURSES:
      ...
      init_terminal_noncurses(prm->inAtty, prm->color, prm->bcolor, prm->col, prm->bgcol, prm->gradient,
                              prm->gradient_count, prm->gradient_colors, audio_raw->width, audio_raw->lines,
                              prm->bar_width);
  ...
  if (prm->fixedbars) {
    prm->autobars = 0;
    if (prm->fixedbars * prm->bar_width + prm->fixedbars * prm->bar_spacing - prm->bar_spacing > audio_raw->width)
      prm->autobars = 1;
  }
  ...
  if (prm->autobars == 1)
    audio_raw->number_of_bars = (*audio_raw->dimension_bar + prm->bar_spacing) / (prm->bar_width + prm->bar_spacing);
  ...
  audio_raw->remainder = (*audio_raw->dimension_bar - audio_raw->number_of_bars * prm->bar_width -
               audio_raw->number_of_bars * prm->bar_spacing + prm->bar_spacing) / 2;
  ...
  debug("height: %d width: %d dimension_bar: %d dimension_value: %d bars:%d bar width: "
                  "%d remainder: %d\n",
        audio_raw->height, audio_raw->width, *audio_raw->dimension_bar, *audio_raw->dimension_value, audio_raw->number_of_bars, prm->bar_width,
        audio_raw->remainder);
#endif

  if (prm->userEQ_enabled && (audio_raw->number_of_bars / audio_raw->output_channels > 0)) {
    audio_raw->userEQ_keys_to_bars_ratio = (double)(((double)prm->userEQ_keys) /
                                                    ((double)(audio_raw->number_of_bars / audio_raw->output_channels)));
  }

  *plan = *cava_init(audio_raw->number_of_bars / audio_raw->output_channels, audio->rate, audio->channels,
                   prm->autosens, prm->noise_reduction, prm->lower_cut_off, prm->upper_cut_off);
  ...
  audio_raw->bars_left = (float *)malloc(audio_raw->number_of_bars / audio_raw->output_channels * sizeof(float));
  audio_raw->bars_right = (float *)malloc(audio_raw->number_of_bars / audio_raw->output_channels * sizeof(float));
  memset(audio_raw->bars_left, 0, sizeof(float) * audio_raw->number_of_bars / audio_raw->output_channels);
  memset(audio_raw->bars_right, 0, sizeof(float) * audio_raw->number_of_bars / audio_raw->output_channels);
  ...
  audio_raw->cava_out = (double *)malloc(audio_raw->number_of_bars * audio->channels / audio_raw->output_channels *
                                        sizeof(double));
  ...
  memset(audio_raw->cava_out, 0, sizeof(double) * audio_raw->number_of_bars * audio->channels / audio_raw->output_channels);
    ...
    for (int n = 0; n < audio_raw->number_of_bars; n++) {
      if (prm->stereo) {
        if (n < audio_raw->number_of_bars / 2)
          cut_off_frequency = plan->cut_off_frequency[audio_raw->number_of_bars / 2 - 1 - n];
        else
      ...
      if (prm->output == OUTPUT_NCURSES) {
        ...
        if (cut_off_frequency < 1000)
          mvprintw(audio_raw->lines, n * (prm->bar_width + prm->bar_spacing) + audio_raw->remainder, "%-4d",
                   freq_floor);
        else if (cut_off_frequency > 1000 && cut_off_frequency < 10000)
                   mvprintw(audio_raw->lines, n * (prm->bar_width + prm->bar_spacing) + audio_raw->remainder, "%.2f",
                   freq_kilohz);
        else
          mvprintw(audio_raw->lines, n * (prm->bar_width + prm->bar_spacing) + audio_raw->remainder, "%.1f",
                   freq_kilohz);
#endif
...
int audio_raw_fetch(struct audio_raw* audio_raw,
                    struct config_params* prm,
                    struct cava_plan* plan,
                    int* re_paint) {
  for (int n = 0; n < (audio_raw->number_of_bars / audio_raw->output_channels) * audio_raw->channels; n++) {
    if (prm->autosens) {
    ...
    for (int n = 0; n < audio_raw->number_of_bars / audio_raw->output_channels; n++) {
      if (prm->userEQ_enabled)
        audio_raw->cava_out[n + audio_raw->number_of_bars / audio_raw->output_channels] *=
          prm->userEQ[(int)floor(((double)n) * audio_raw->userEQ_keys_to_bars_ratio)];
      audio_raw->bars_right[n] = audio_raw->cava_out[n + audio_raw->number_of_bars / audio_raw->output_channels];
    }
  } else {
    for (int n = 0; n < audio_raw->number_of_bars; n++) {
      if (prm->userEQ_enabled)
  ...
  if (prm->monstercat) {
    if (audio_raw->channels == 2) {
      audio_raw->bars_left =
        monstercat_filter(audio_raw->bars_left, audio_raw->number_of_bars / 2, prm->waves, prm->monstercat);
      audio_raw->bars_right = monstercat_filter(audio_raw->bars_right, audio_raw->number_of_bars / 2, prm->waves,
                                               prm->monstercat);
    } else {
      audio_raw->bars_raw =
        monstercat_filter(audio_raw->bars_raw, audio_raw->number_of_bars, prm->waves, prm->monstercat);
    }
  }
  if (audio_raw->channels == 2) {
    if (prm->stereo) {
      ...
      for (int n = 0; n < audio_raw->number_of_bars; n++) {
        if (prm->reverse) {
          if (prm->mono_opt == AVERAGE) {
            ...
            audio_raw->bars_raw[audio_raw->number_of_bars - n - 1] = audio_raw->bars_left[n];
          } else if (prm->mono_opt == RIGHT) {
            audio_raw->bars_raw[audio_raw->number_of_bars - n - 1] = audio_raw->bars_right[n];
          }
        } else {
          if (prm->mono_opt == AVERAGE) {
            audio_raw->bars_raw[n] = (audio_raw->bars_left[n] + audio_raw->bars_right[n]) / 2;
          } else if (prm->mono_opt == LEFT) {
            audio_raw->bars_raw[n] = audio_raw->bars_left[n];
          } else if (prm->mono_opt == RIGHT) {
